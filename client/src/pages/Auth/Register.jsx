...
const Register = () => {
  ...
  return (
    <div className="min-h-screen flex">
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-surface-800 to-surface-950 text-white flex-col items-center justify-center p-12">
        <ChefHat className="w-16 h-16 mb-6 text-brand-400" />
        <h1 className="font-display text-4xl font-bold mb-4 text-center">Join RecipeNest</h1>
        <p className="text-surface-400 text-lg text-center max-w-xs">Share your passion for food with millions of food lovers.</p>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 overflow-y-auto">
        <div className="w-full max-w-sm py-8">
          ...
          <p className="text-surface-500 text-sm mb-6">Already have an account? <Link to="/auth/login" className="text-brand-600 hover:underline">Sign in</Link></p>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { value: 'user', label: '🍽️ Food Lover', desc: 'Discover & save recipes' },
                  { value: 'chef', label: '👨‍🍳 Chef', desc: 'Create & share recipes' },
                  ...
                  <label key={value} className="cursor-pointer">
                    ...
                    <div className={`border-2 rounded-xl p-3 text-center transition-all ${
                      selectedRole === value 
                        ...
                        : (errors.role ? 'border-red-300' : 'border-surface-200 hover:border-brand-400')
                    }`}>
                      <div className={`font-medium text-sm ${selectedRole === value ? 'text-brand-700' : ''}`}>{label}</div>
